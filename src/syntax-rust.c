...
syntax_highlight_rust(SyntaxContext *ctx, PangoAttrList *attrs, const char *text, size_t len, SyntaxState state, size_t line_index)
{
    ...
    while (cur < len) {
        ...
        if (state == STATE_IN_DOUBLE_QUOTE) {
            ...
            while (cur < len) {
                if (text[cur] == '"') {
                    add_color_attr(ctx, attrs, start_pos, cur + 1, COLOR_STRING); 
                    cur++;
                    state = STATE_ROOT;
                    start_pos = cur; 
                    break;
        ...
        if (state == STATE_ROOT) {
            ...
            if (text[cur] == '/' && cur+1 < len) {
                ...
                if (text[cur+1] == '*') {
                    ...
                    cur += 2;
                    add_color_attr(ctx, attrs, cur - 2, cur, COLOR_COMMENT);
                    continue;
                }
            }
            ...
            if (text[cur] == 'r' && cur + 1 < len && (text[cur+1] == '#' || text[cur+1] == '"')) {
                if (text[cur+1] == '"' || (cur+2 < len && text[cur+1] == '#' && text[cur+2] == '"')) {
                     add_color_attr(ctx, attrs, cur, len, COLOR_STRING); 
                     cur = len; 
                     continue;
                }
            }
            ...
            if (text[cur] == '\'') {
                ...
                if (cur + 1 < len && (g_ascii_isalpha(text[cur+1]) || text[cur+1] == '_')) {
                     ...
                     if (is_char) {
                         p = cur + 1;
                         if (text[p] == '\\') {
                              add_color_attr(ctx, attrs, start_pos, p, COLOR_STRING);
                              add_color_attr(ctx, attrs, p, p+2, COLOR_BUILTIN); 
                              p += 2;
                              add_color_attr(ctx, attrs, p, p+1, COLOR_STRING); 
                              cur = p + 1;
                         } else {
                              cur = p + 1;
                              add_color_attr(ctx, attrs, start_pos, cur, COLOR_STRING);
                    ...
                    if (p < len && text[p] == '\'') {
                        add_color_attr(ctx, attrs, start_pos, cur+1, COLOR_STRING); 
                        add_color_attr(ctx, attrs, cur+1, cur+3, COLOR_BUILTIN); 
                        add_color_attr(ctx, attrs, cur+3, cur+4, COLOR_STRING); 
                        cur = cur + 4;
                        continue;
                    }
                }
                ...
                if (cur < len && text[cur] == '[') {
                     ...
                     while (cur < len && text[cur] != ']') {
                         ...
                         if (text[cur] == '"') {
                             ...
                             if (cur < len) cur++;
                             add_color_attr(ctx, attrs, s_start, cur, COLOR_STRING);
                             continue;
                         }
            ...
            if (g_ascii_isalpha(text[cur]) || text[cur] == '_') {
                ...
                if (is_word_in_list(word, word_len, rust_keywords)) {
                     ...
                     add_color_attr(ctx, attrs, start_pos, cur, COLOR_CONSTANT);
