...
syntax_highlight_desktop(SyntaxContext *ctx, PangoAttrList *attrs, const char *text, size_t len, SyntaxState state, size_t line_index)
{
    ...
    while (cur < len) {
        ...
        if (text[cur] == '[') {
            ...
            add_color_attr(ctx, attrs, cur, cur+1, COLOR_NUMBER); /* [ -> orange */
            cur++;
            ...
            if (cur > name_start) {
                add_color_attr(ctx, attrs, name_start, cur, COLOR_FUNCTION); /* Name -> Cyan */
            }
        ...
        if (p < len && text[p] == '=') {
            /* Found key */
            add_color_attr(ctx, attrs, cur, p, COLOR_TAG); /* Key -> Red (or Tag color) */
            
            /* Highlight = as Cyan */
            add_color_attr(ctx, attrs, p, p+1, COLOR_BUILTIN); 
            ...
            while (cur < len) {
                if (text[cur] == ';') {
                    if (cur > val_start) add_color_attr(ctx, attrs, val_start, cur, COLOR_STRING);
