...
static const char *c_control_keywords[] = {
    ...
    "for", "goto", "if", "return", "switch", "while", NULL
};
...
syntax_highlight_c(SyntaxContext *ctx, PangoAttrList *attrs, const char *text, size_t len, SyntaxState state, size_t line_index)
{
    ...
    if (!attrs) {
        size_t cur = 0;
        int paren_depth = 0;
        int bracket_depth = get_line_start_bracket_depth(ctx, line_index);
        ...
        set_line_end_state_with_depth(ctx, line_index, state, bracket_depth);
        return;
    }


    size_t cur = 0;
    int paren_depth = 0; /* specifically for params */
    int bracket_depth = get_line_start_bracket_depth(ctx, line_index); /* for bracket pair colorization */
    while (cur < len) {
        ...
        if (state == STATE_C_PARAMS) {
            ...
            if (strchr(")]}", text[cur])) {
                 if (text[cur] == ')' && paren_depth > 0) paren_depth--;
