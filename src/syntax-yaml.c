    ...
    while (cur < len) {
        ...
        if (!key_scanned) {
            ...
            if (found_colon && !complex_key) {
                ...
                add_color_attr(ctx, attrs, cur, probe, COLOR_VARIABLE);
                cur = probe + 1; /* Skip colon */
                
                /* Highlight the colon itself as Punctuation (Orange) per request */
                add_color_attr(ctx, attrs, probe, probe + 1, COLOR_PUNCTUATION);
        ...
        if (text[cur] == '!' && cur + 1 < len && text[cur+1] == '!') {
             ...
             add_color_attr(ctx, attrs, start, cur, COLOR_KEYWORD); /* Purple */
             continue;
        }
        ...
        if (text[cur] == '*' || text[cur] == '&') {
             if (cur+1 < len && !g_ascii_isspace(text[cur+1])) {
                 size_t start = cur;
                 cur++;
                 add_color_attr(ctx, attrs, start, cur, COLOR_KEYWORD); /* Prefix: Purple */
                 
                 size_t name_start = cur;
                 while (cur < len && (g_ascii_isalnum(text[cur]) || text[cur] == '-' || text[cur] == '_')) cur++;
                 add_color_attr(ctx, attrs, name_start, cur, COLOR_TYPE); /* Name: Yellow */
                 continue;
             }
        }
        ...
        /* Brackets */
        if (strchr("[]{}()", text[cur])) {
             add_color_attr(ctx, attrs, cur, cur+1, COLOR_PUNCTUATION); /* Orange */
             cur++;
             continue;
        }
        ...
        if (text[cur] == '"') {
            ...
            while (cur < len) {
                if (text[cur] == '"') {
                    if (cur > seg_start) add_color_attr(ctx, attrs, seg_start, cur, COLOR_STRING);
                    add_color_attr(ctx, attrs, cur, cur+1, COLOR_STRING); /* Close quote */
                    cur++;
                    break;
                }
                ...
                if (text[cur] == '\\') {
                    ...
                    add_color_attr(ctx, attrs, cur, esc_end, COLOR_BUILTIN); /* Cyan */
                    cur = esc_end;
                    seg_start = cur;
                    continue;
                }
                ...
                if (g_ascii_isdigit(text[cur])) {
                     ...
                     add_color_attr(ctx, attrs, num_start, cur, COLOR_NUMBER); /* Orange */
                     seg_start = cur;
                     continue;
                }
        ...
        if (g_ascii_isdigit(text[cur]) || text[cur] == '-' || text[cur] == '.') {
            ...
            if (is_number_special) {
                ...
                add_color_attr(ctx, attrs, start_pos, probe, COLOR_NUMBER);
                cur = probe;
                continue;
            }
        ...
        if (text[cur] == '$' && cur + 1 < len && text[cur+1] == '{') {
            size_t start = cur;
            cur += 2;
            add_color_attr(ctx, attrs, start, cur, COLOR_PREPROC); /* ${ -> Purple */
            
            size_t var_start = cur;
            while (cur < len && text[cur] != '}') cur++;
            add_color_attr(ctx, attrs, var_start, cur, COLOR_VARIABLE); /* XXXX -> Red */
            
            if (cur < len) {
                add_color_attr(ctx, attrs, cur, cur+1, COLOR_PREPROC); /* } -> Purple */
                cur++; 
            }
            continue;
        }
        ...
        if (g_ascii_isalnum(text[cur]) || strchr("._-/+@=^%", text[cur])) {
            ...
            if (delim) {
                if ((word_len == 4 && strncmp(text+start_pos, "true", 4) == 0) ||
                    ...
                    add_color_attr(ctx, attrs, start_pos, cur, COLOR_NUMBER); /* Orange */
                } else {
                    add_color_attr(ctx, attrs, start_pos, cur, COLOR_STRING);
                }
            } else {
